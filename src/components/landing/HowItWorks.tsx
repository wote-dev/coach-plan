export default function HowItWorks() {
  return (
    <section id="how-it-works" className="relative border-y border-white/10 py-28 overflow-hidden">
      {/* Section background image with subtle overlay */}
      <div className="absolute inset-0 -z-10">
        <div
          className="absolute inset-0"
          style={{
            backgroundImage: "url('/tennis6.jpg')",
            backgroundSize: 'cover',
            backgroundPosition: 'center',
          }}
        />
        <div className="absolute inset-0 bg-[#060D16]/80" />
      </div>
      ...
      <div className="mx-auto grid max-w-6xl gap-12 px-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
        <div className="space-y-6">
          ...
          <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#070F1A]/80 p-6 text-sm text-white/65">
            <div
            ...
            />
            <div className="absolute inset-0 -z-10 bg-[#070F1A]/70" />
        ...
        <ol className="space-y-5">
          ...
          {[
            ...
            <li
              key={step.title}
              className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#070F1A]/70 p-6 text-white/80"
            >
              ...
              <span className="mb-4 inline-flex h-9 w-9 items-center justify-center rounded-full bg-[#CCFF00]/20 text-sm font-semibold text-[#CCFF00]">
                {index + 1}
              </span>
              <h3 className="text-lg font-semibold text-white">{step.title}</h3>
              <p className="mt-2 text-sm">{step.desc}</p>
              <p className="mt-4 text-xs text-white/60">{step.detail}</p>
            </li>
          ))}
        </ol>
      </div>
